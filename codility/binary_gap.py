...
# has binary representation 1000010001 and contains two binary gaps: one of length 4 and one of length 3. The
...
def binaryGap(number):
    binaryNumber = str(bin(number).strip('0b'))
    differenceList = []
    onesPositionList = [i.start() for i in re.finditer('1', binaryNumber)]
    for one in range(len(onesPositionList)):
        if one < len(onesPositionList)-1:
            difference = onesPositionList[one + 1] - onesPositionList[one]
            if difference > 0:
                differenceList.append(difference - 1)
